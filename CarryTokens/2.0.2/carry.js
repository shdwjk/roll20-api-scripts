/* globals HtmlBuilder */
const CarryTokens = (() => { //eslint-disable-line no-unused-vars
  ...
  const scriptName = 'carrytokens';
  const version = '2.0.2';
  const schemaVersion = 0.1;
  const lastUpdate = 1732066117;
  ...
  const cleanupState = ()=>{
    // cleanup carried that are gone
    Object.keys(state[scriptName].carried).forEach(id=>{
      let c = getObj('graphic',id);
      if(!c){
        delete state[scriptName].carried[id];
      }
    });

    // cleanup carriers that are gone
    Object.keys(state[scriptName].carriers).forEach(id=>{
      let c = getObj('graphic',id);
      if(!c){
        delete state[scriptName].carriers[id];
      } else {
        state[scriptName].carriers[id] = state[scriptName].carriers[id].filter(id=>getObj('graphic',id));
        if( state[scriptName].carriers[id].length === 0){
          delete state[scriptName].carriers[id];
        }
      }
    });
  ...
  function _carry(carrier, token) {
    ...
    state[scriptName].carriers[carrier.id]=[...new Set([...(state[scriptName].carriers[carrier.id]??[]),token.id])];
    state[scriptName].carried[token.id]=carrier.id;
  }
  ...
  function drop(carrier, token) {
    if(!state[scriptName].carriers.hasOwnProperty(carrier.id))
      return;

    state[scriptName].carriers[carrier.id]=state[scriptName].carriers[carrier.id].filter(id=>id!==token.id);
    delete state[scriptName].carried[token.id];
  }
  ...
  function dropAll(carrier) {
    if(state[scriptName].carriers.hasOwnProperty(carrier.id)) {
      state[scriptName].carriers[carrier.id].forEach(id=>delete state[scriptName].carried[id]);
      delete state[scriptName].carriers[carrier.id];
    }
  }
  ...
  function getCarriedTokens(carrier) {
    return [...state[scriptName].carriers[carrier.id]||[]].map(id=>getObj('graphic',id)).filter(g=>undefined!==g);
  }
  ...
  function moveCarriedTokens(carrier) {
    ...
    let carrying = getCarriedTokens(carrier);
    if(carrying)
      carrying.forEach(carried => {
  ...
  on("change:graphic", obj => {
    try {
      ...
      // stop carrying it.
      if(state[scriptName].carried.hasOwnProperty(obj.id)) {
        let carrier = getObj('graphic',state[scriptName].carried[obj.id]);
        drop(carrier, obj);
      }
